                ...
                <div class="filter-controls">
                    ...
                    <div class="filter-buttons">
                        <button type="button" class="filter-btn active" data-filter="all">
                            ...
                            All Rooms
                            <span class="badge badge-light">{{ $rooms->count() }}</span>
                        </button>
                        <button type="button" class="filter-btn" data-filter="classroom">
                            <i class="fas fa-door-open mr-1"></i>
                            Classrooms
                            <span class="badge badge-primary">{{ $rooms->where('is_lab', false)->count() }}</span>
                        </button>
                        <button type="button" class="filter-btn" data-filter="laboratory">
                            <i class="fas fa-flask mr-1"></i>
                            Laboratories
                            <span class="badge badge-info">{{ $rooms->where('is_lab', true)->count() }}</span>
                        </button>
                    </div>
                </div>
            </div>
    ...
    <div class="rooms-grid" id="roomsContainer">
        @foreach($rooms as $room)
            <div class="room-card room-item" 
                 data-room-type="{{ $room->is_lab ? 'laboratory' : 'classroom' }}" 
                 data-room-name="{{ strtolower($room->name) }}" 
                 data-room-description="{{ strtolower($room->description ?? '') }}">
                <div class="room-header">
                    ...
                    <div class="room-header-actions">
                        <button onclick="showQRCode({{ $room->id }}, '{{ $room->name }}')" class="btn btn-outline-success btn-sm" title="View QR Code">
                ...
                <div class="room-footer">
                    <a href="{{ route('admin.room-management.room-timetables.show', $room->id) }}" class="btn btn-primary btn-view-timetable">
...
function printAllRoomTimetables() {
    ...
    const roomUrls = [];
        ...
        visibleRoomItems.each(function() {
            ...
            roomUrls.push('{{ url('/admin/room-management/room-timetables') }}/' + roomId);
        });
