...
<nav aria-label="breadcrumb" class="mb-3">
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active" aria-current="page">
            <i class="fas fa-{{ $programType == 'senior_high' ? 'graduation-cap' : 'university' }}"></i>
            {{ $programType == 'senior_high' ? 'Senior High School' : 'College' }} Classes
        </li>
    </ol>
</nav>
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h3 class="card-title">
                    <i class="fas fa-{{ $programType == 'senior_high' ? 'graduation-cap' : 'university' }}"></i>
                    {{ $programType == 'senior_high' ? 'Senior High School Classes' : 'College Classes' }}
                </h3>
            ...
            <div class="card-body p-2">
                <form method="GET" action="{{ route('admin.school-classes.program', $programType) }}" class="mb-3">
                    <div class="form-row align-items-end">
                        <div class="col-md-4 mb-2">
                            <label for="q" class="small text-muted">Search Classes</label>
                            <input type="text" name="q" id="q" value="{{ $search ?? '' }}" class="form-control" placeholder="Search by class name or section">
                        </div>
                        <div class="col-md-3 mb-2">
                            ...
                            <select name="program_id" id="program_id" class="form-control" onchange="this.form.submit()">
                                ...
                                @foreach($programs as $p)
                                    <option value="{{ $p->id }}" {{ (isset($programId) && (string)$programId === (string)$p->id) ? 'selected' : '' }}>{{ $p->name }}</option>
                                @endforeach
                            </select>
                        </div>
                ...
                @foreach($programs as $program)
                    <div class="program-section mb-3">
                        ...
                        <div class="row">
                            @foreach($gradeLevels->where('program_id', $program->id) as $gradeLevel)
                                @php
